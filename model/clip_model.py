...
logger = logging.getLogger("CRST.model")
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: Tuple[int, int], patch_size: int, stride_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        self.resolution_embedding = nn.Parameter(scale * torch.randn(4, width))
    ...
    def forward(self, x: torch.Tensor, res_label: torch.Tensor = None):
        ...
        if res_label is not None:
            res_embed = self.resolution_embedding[res_label].to(x.dtype)
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, res_label=None):
        return self.visual(image.type(self.dtype), res_label)
    ...
    def forward(self, image, text, res_label=None):
        image_features = self.encode_image(image, res_label)
