...
@lru_cache()
def default_bpe():
    bpe_path = os.environ.get("CRST_BPE_PATH")
    if bpe_path:
        return bpe_path

    bpe_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../data/bpe_simple_vocab_16e6.txt.gz")
    ...
    try:
        print(f"[CRST] Downloading CLIP BPE vocabulary to {bpe_path}")
        urllib.request.urlretrieve(url, bpe_path)
    except Exception as exc:
        raise FileNotFoundError(
            "Missing CLIP BPE vocabulary. Please download bpe_simple_vocab_16e6.txt.gz "
            f"and place it at {bpe_path}, or set CRST_BPE_PATH. Original error: {exc}"
        )
    return bpe_path
